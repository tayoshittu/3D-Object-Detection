...
""" Training routine for 3D object detection with SUN RGB-D."""

# Sample usage:
# python train.py --log_dir log_sunrgbd
...
from tf_visualizer import log_writing
...
print('Testing Dataloader', len(TEST_DATALOADER))


# Init the model and optimzier
MODEL = importlib.import_module('detectnet') # import network module
...
# TFBoard Visualizers
TRAIN_VISUALIZER = log_writing(FLAGS, 'train')
TEST_VISUALIZER = log_writing(FLAGS, 'test')
...
if __name__=='__main__':
    ...
    for epoch in range(start_epoch, MAX_EPOCH):
        ...
        if EPOCH_CNT == 0 or EPOCH_CNT % 10 == 9:  # Eval every 10 epochs
            stat_dict = {}  # collect statistics
            ap_Measurement = AP_Measurement(ap_iou_thresh=FLAGS.ap_iou_thresh,
            ...
            for key in sorted(stat_dict.keys()):
                print('eval mean %s: %f' % (key, stat_dict[key] / (float(batch_idx + 1))))

            # Evaluate average precision
            metrics_dict = ap_Measurement.compute_metrics()
